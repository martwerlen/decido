...
export default function DecisionAdminClient({
  ...
  const handleAddProposal = async () => {
    ...
    try {
      const response = await fetch(
        `/api/organizations/${orgId}/decisions/${decision.id}/proposals`,
        {
          method: 'POST',
  ...
  const handleDeleteProposal = async (proposalId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/organizations/${orgId}/decisions/${decision.id}/proposals/${proposalId}`,
        { method: 'DELETE' }
      );
  ...
  const handleUpdateAmendedProposal = async () => {
    ...
    try {
      const response = await fetch(
        `/api/organizations/${orgId}/decisions/${decision.id}`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleAddParticipants = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/organizations/${orgId}/decisions/${decision.id}/participants`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteParticipant = async (participantId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/organizations/${orgId}/decisions/${decision.id}/participants?participantId=${participantId}`,
        { method: 'DELETE' }
      );
  ...
  const handleLaunchDecision = async () => {
    ...
    try {
      const response = await fetch(
        `/api/organizations/${orgId}/decisions/${decision.id}/launch`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        }
      ...
      setTimeout(() => {
        router.push(`/organizations/${orgId}/decisions/${decision.id}/vote`);
      }, 2000);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur');
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="flex gap-4">
        <button
          onClick={() => router.push(`/organizations/${orgId}/decisions`)}
