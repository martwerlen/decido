...
export default function VotePageClient({
  ...
  const handleVoteProposal = async () => {
    ...
    try {
      const response = await fetch(
        `/api/organizations/${orgId}/decisions/${decision.id}/vote`,
        {
          method: 'POST',
  ...
  const handleVoteConsensus = async (value: 'AGREE' | 'DISAGREE') => {
    ...
    try {
      ...
      if (consensusReached) {
        ...
        setTimeout(() => {
          router.push(`/organizations/${orgId}/decisions/${decision.id}/results`);
        }, 2000);
      } else {
        setSuccess('Votre vote a été enregistré !');
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await fetch(
        `/api/organizations/${orgId}/decisions/${decision.id}/comments`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleUpdateComment = async (commentId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/organizations/${orgId}/decisions/${decision.id}/comments/${commentId}`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      {!isOpen && (
        <div className="bg-yellow-50 border border-yellow-200 px-4 py-3 rounded mb-4">
          <p className="text-yellow-800">
            ...
            <Link
              href={`/organizations/${orgId}/decisions/${decision.id}/results`}
              className="underline"
            >
              Voir les résultats
            </Link>
      ...
      <div className="flex gap-4 mt-6">
        <Link
          href={`/organizations/${orgId}/decisions`}
        ...
        {isCreator && (
          <Link
            href={`/organizations/${orgId}/decisions/${decision.id}/admin`}
