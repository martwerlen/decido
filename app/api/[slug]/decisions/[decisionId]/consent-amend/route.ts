/**
 * API pour amender la proposition (décisions par consentement)
 * Uniquement le créateur peut amender, et uniquement pendant le stade AMENDEMENTS
 */
 ...
 * Amender la proposition et passer au stade OBJECTIONS
 */
export async function PATCH(
  ...
  try {
    ...
    const { amendedProposal } = body

    if (!amendedProposal || amendedProposal.trim() === '') {
      return NextResponse.json({ error: 'La proposition amendée est requise' }, { status: 400 })
    }
    ...
    if (decision.creatorId !== session.user.id) {
      return NextResponse.json({ error: 'Seul le créateur peut amender la proposition' }, { status: 403 })
    }

    if (decision.consentCurrentStage !== 'AMENDEMENTS') {
      return NextResponse.json({ error: 'Les amendements ne sont possibles que pendant le stade AMENDEMENTS' }, { status: 400 })
    }
