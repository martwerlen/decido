...
// POST /api/organizations/[orgId]/decisions/[decisionId]/launch - Lance une décision
  ...
  try {
    ...
    if (decision.votingMode === 'INVITED') {
      const emailPromises = decision.participants.map(async (participant) => {
        ...
        const voteUrl = `${process.env.NEXTAUTH_URL}/organizations/${orgId}/decisions/${decisionId}/vote`;
